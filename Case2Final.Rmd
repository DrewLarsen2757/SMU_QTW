...
From the age distribution below we can see the participant's age goes from 10 to 70 and most of the participant are between 20-40. The data is skew to the right so we have some other participant form 40 to 60.
...
The scatter plot below is across years doesn't show any particular linear trend but show a big cluster which tell us year by year the most of the participant are between age 25 and 50 and runtime are between 75 and 125 min.
...
The correlation plot below is between the age and run time, confirmed on the idea of weak correlation between the two variable.
...
As you can see from the plot below the number of participant is increasing every year, from 1999 to 2012 there are almost 4X the number of participant growing from 2500 to 10000.
...
From the scatter plot below we can see there's not a clear linear trend or correlation between the age and the run time. but confirm on the big cluster.
