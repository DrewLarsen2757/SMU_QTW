...
After our initial analysis, it was determined that the best model included mac addresses: (mac addresses used)
We decided to look into an alternate model using these mac addresses. Our alternative model is a weighted KNN model. This model takes the root average squared error between the values of the mac address for the position we are trying to predict and the mac address for the values of our training set. In this case, the position we are trying to predict is the online data set and the training set is the offline data. We take the _____ (k) positions with the lowest root average squared errors, since it was determined that this was the best number for k in our initial analysis. The root average squared error is used to create weights for these _____ (k) positions, where the lowest root average squared error has more impact on our predictions. We multiply these weights by their correspoinding x and y positions, sum the positions and are left with predictions for the positions of the online data. The root average squared error of this model is _____ for the x value and ____ for the y value. This performs (better / worse) than our best non-weighted KNN model. 
