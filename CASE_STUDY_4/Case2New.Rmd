...
IGNORE: Scrape 1999 data this will output els99  a character string with the 1999 women data you need
```{r}
# 1999
url <- urls[1]
...
2011 handling: invalid multibyte string. Handled in initial call; had to change encoding to latin1
```{r}
table = extractVariables(wTables3$`2006`)
```
...
sapply(womendf, function(x) sum(is.na(x$runTime)))
```
It looks like the time is in the home column, for some reason. It's because there's no break in the = signs between hometown and net time. 64 needs to be turned into a blank
```{r}
head(womendf$`2006`)
womenResMat$`2006`
time = str(Res[,2], '\((\\D\\s{1,})\\D)\\D\\s{1,})')

time = str_extract(womenResMat$`2006`[,2], '\\d')
time = unlist(time)
useTime = time[seq(2,length(time), 2)]
loc = time[seq(1,length(time), 2)]
useTime[5434]
length(useTime)
```
...
```{r}
which(womendf$`2011`$runTime == max(womendf$`2011`$runTime))
wTables3$`2011`[7785+8]
womendf$`2011`$runTime[7785] = 127.65
```
...
```{r}
womendf$`2003`$home = substring(womendf$`2003`$home, 3)
```
...
################################################################
To Remove NA's for age analysis 
```{r}
womenTimeMat = sapply(womenResMat,function(x)  na.omit(x))
```
...
Values = mapply(substr, list(body), start = locCols[1, ], stop = locCols[2, ])
colnames(Values) = varNames
Values = Values[grep('^#', Values[,3], invert = TRUE)]
invisible(Values)
