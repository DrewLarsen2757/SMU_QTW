...
From the runTime distribution below we can see that the runtime is normally distributed from 1999-2012. This means that the expected female runtime is around 98 Min.
...
```
**Age Distribution:**
From the age distribution above we can see the participant's age goes from 10 to 70 and most of the participant are between 20-40. the data is skew to the right so we have some other participant form 40 to 60
```{r}
hist(df$age, xlab = 'Age', main = 'Distribution of age across Female Runners', col = 'red', breaks = 10)
```
**Scatterplot:** 
The scatter plot above across year doesn't show any particular linear trend but show a big cluster which tell us year by year the most of the participant are between age 25 and 50 and runtime are between 75 and 125 min.
```{r}
...
**Correlation Plot:** 
The correlation plot between the age and runTime, confirmed on the idea of weak correlation between the two variable
```{r}
#dot correlation
...
```
**Bar Plot of # of people per race:** 
As you can see from the plot above the number of participant increase every year, from 1999 to 2012 we have almost 4X the number of participant from 2500 to 10000.
```{r}
ggplot(df, aes(year)) +  geom_bar(fill = "#0073C2FF")

```
**Scatterplot of runtime vs. age, not colored:**
From the plot above we can see there's not a clear linear trend or correlation between the age and the runtime. but confirm on the big cluster.
```{r}
...
Looking at the violin below, there isn't a lot of evidence that the mean age has changed year over year. We will run a more formal analysis to see if this is true or not.
...
Looking at the violin plots below, it doesn't look as if run time has changed over time. We run a more formal statistical analysis to check if times have changed statistically. 
```{r}
...
Based on our analysis of age and runner time we see statistically significant differences in both time and age but we do not believe that the differences are large enough to recommend or warrant any major changes to the Cherry Blossom 10 mile Race. We would recomend that given more time further anlysis of runner home town/location could highlight new insights into questions such as where are most of the runners from, what countries are represented, and how popular is the race internationally. As we have seen in the summary data the race itself is increasing in participation and has grown by more than 500 participants a year over the 14 year period from 1999 to 2012. We would recomend that based on the growth in popularity of the race considerations should be taken to add more facilites, water and water stations, gear/givaways, and make sure that any concessions are approriatly stocked for the increase in runners.
